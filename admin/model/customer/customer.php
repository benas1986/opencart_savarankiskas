...
class ModelCustomerCustomer extends Model {
    ...
    public function deleteCustomer($customer_id) {
        ...
        // if customer deleted, delete and imported prices 
        $this->db->query("DELETE FROM " . DB_PREFIX . "customer_import WHERE customer_id = '" . (int)$customer_id . "'");
        $this->db->query("DELETE FROM " . DB_PREFIX . "address WHERE customer_id = '" . (int)$customer_id . "'");
    }
    ...
    public function getCustomerName($customer_id) {
        $query = $this->db->query("SELECT CONCAT(c.firstname, ' ', c.lastname) AS name FROM " . DB_PREFIX . "customer c WHERE c.customer_id ='".(int)$customer_id. "'" );
    ...
    public function editImport($customer_import_id, $product_ean_code = '', $special_price_eur = '' ) {
        if ($customer_import_id!=''){

        }
        $this->db->query("UPDATE " . DB_PREFIX . "customer_import SET product_ean_code = '" . (int)$product_ean_code . "', special_price_eur = '" .  (double)$special_price_eur . "' WHERE customer_import_id = '" . (int)$customer_import_id . "'");
    }

    public function deleteImport($customer_import_id) {
        $query = $this->db->query("DELETE FROM " . DB_PREFIX . "customer_import WHERE customer_import_id = '" . (int)$customer_import_id . "'");
    }

    public function getImportByImportId($customer_import_id) {
        $customer_import_data = array();

        $query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "customer_import WHERE customer_import_id = '" . (int)$customer_import_id . "'");
        ...
        foreach ($query->rows as $result){
            $customer_import_data = array(
                ...
                'special_price_eur' => $this->currency->format($result['special_price_eur'], $this->config->get('config_currency'))    
            );
        }
        return $customer_import_data;
    }

    public function getImportNameByImportId ($customer_import_id) {     
        $query = $this->db->query("SELECT pd.name as product_name FROM (" . DB_PREFIX . "product pr LEFT JOIN " . DB_PREFIX . "product_description pd ON pd.product_id = pr.product_id), (" . DB_PREFIX . "customer_import ci LEFT JOIN " . DB_PREFIX . "product ON " . DB_PREFIX . "product.ean = ci.product_ean_code) WHERE customer_import_id = '" . (int)$customer_import_id . "' AND pd.product_id = pr.product_id AND pr.ean =ci.product_ean_code ");
    ...
    public function getImport($customer_import_id) {
        $query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "customer_import WHERE customer_import_id = '" . (int)$customer_import_id . "'");
    ...
    public function getImports($customer_id, $start = 0, $limit = 10) {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer_import WHERE customer_id = '" . (int)$customer_id . "' ORDER BY customer_import_id ASC LIMIT " . (int)$start . "," . (int)$limit);
    ...
    public function getLatestImportDate($customer_id) {
        $import_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer_import WHERE customer_id = '" . (int)$customer_id . "'");

        if ($import_query->num_rows) {
            $query = $this->db->query("SELECT date_added AS date FROM " . DB_PREFIX . "customer_import WHERE customer_id = '" . (int)$customer_id . "'ORDER BY date_added DESC LIMIT 1");

            if ($query->num_rows) {
                $latest_date = $query->row['date'];
                return $latest_date;
            } else {
                $latest_date = '';
                return $latest_date;
    ...
    public function importCSV($customer_id, $data) {

        $product_ean_code = $data[0];
        $special_price_eur = $data[1];
        
        if ($data[1] != "") {
            $this->db->query("INSERT INTO ".DB_PREFIX."customer_import SET customer_id='". $customer_id . "', product_ean_code = '". (int)$product_ean_code . "', special_price_eur='".  (double)$special_price_eur  . "', date_added = NOW() ");
        } else {
            $this->db->query("INSERT INTO ".DB_PREFIX."customer_import SET customer_id='". $customer_id . "', product_ean_code = '". (int)$product_ean_code . "', special_price_eur = NULL, date_added = NOW() ");
        }
    }  

    public function deleteAllSpecialPrices() {
        $this->db->query("TRUNCATE TABLE " .DB_PREFIX. "customer_import "); 
    }  
}