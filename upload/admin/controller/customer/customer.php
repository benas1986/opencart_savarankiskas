...
class ControllerCustomerCustomer extends Controller {
    ...
    public function import() {
        ...
        foreach ($results as $result) {
            $data['imports'][] = array(
                'customer_id'           => $result['customer_id'],
                'customer_import_id'    => $result['customer_import_id'],
                'product_ean_code'      => $result['product_ean_code'],
        ...
        // find Customer Name
        $customer_name_to_view = $this->model_customer_customer->getCustomerName($this->request->get['customer_id']);

        $data['text_import'] = sprintf($this->language->get('text_import'), $customer_name_to_view);

        // find Latest import date
        $latest_date = $this->model_customer_customer->getLatestImportDate($this->request->get['customer_id']);

        if ($latest_date != '') {
            $data['latest_date'] = sprintf($latest_date);
        } else {
            $data['latest_date'] = sprintf('');
        }

        // find user name which last upload special prices
        $data['username'] = $this->user->getUserName();

        // upload csv file  
        $data['import_csv'] = $this->url->link('customer/customer/importCSV', 'user_token=' . $this->session->data['user_token'] . '&customer_id=' . $this->request->get['customer_id'] , true);

        /*$data['action'] = $this->url->link('customer/customer/importCSV', 'user_token=' . $this->session->data['user_token'] . '&customer_id=' . $result['customer_id'], true);*/
    ...
    public function editImport() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateImportForm()) {
            ...
            $this->model_customer_customer->editImport($this->request->get['customer_import_id'], $this->request->post['product_ean_code'], $this->request->post['special_price_eur']);
    ...
    protected function getImportForm() {
        ...
        if (isset($this->error['product_ean_code'])) {
            $data['error_product_ean_code'] = $this->error['product_ean_code'];
        } else {
            $data['error_product_ean_code'] = '';
        } 

        if (isset($this->error['special_price_code'])) {
            $data['error_special_price_code'] = $this->error['special_price_code'];
        } else {
            $data['error_special_price_code'] = '';
        } 
        ...
        $data['breadcrumbs'][] = array(
            'text' => $this->language->get('heading_title'),
            'href' => $this->url->link('customer/customer/edit', 'user_token=' . $this->session->data['user_token'] . $url, true)
        );
        ...
        $data['imports'] = $this->model_customer_customer->getImport($this->request->get['customer_import_id']);

        $import_info = $this->model_customer_customer->getImportByImportId('customer_import_id');

        if (isset($this->request->post['product_ean_code'])) {
            $data['product_ean_code'] = $this->request->post['product_ean_code'];
        } elseif (!empty($import_info)) {
            $data['product_ean_code'] = $import_info['product_ean_code'];
        } else {
            $data['product_ean_code'] = " ";
        }

        if (isset($this->request->post['special_price_code'])) {
            $data['special_price_code'] = $this->request->post['special_price_code'];
        } elseif (!empty($import_info)) {
            $data['special_price_code'] = $import_info['special_price_code'];
        } else {
            $data['special_price_code'] = " ";
        }
        ...
        $this->response->setOutput($this->load->view('customer/customer_import_edit_form', $data));
    }
    ...
    protected function validateImportForm() {
        ...
        if ((utf8_strlen($this->request->post['product_ean_code']) < 1) || (utf8_strlen(trim($this->request->post['product_ean_code'])) > 11)) {
            $this->error['product_ean_code'] = $this->language->get('error_product_ean_code');
        }
    ...
    public function importCSV(){
        ...
        $data['text_form'] = $this->language->get('text_import_csv');
        ...
        $data['breadcrumbs'][] = array(
            'text' => $this->language->get('heading_title'),
            'href' => $this->url->link('customer/customer/ImportCSV', 'user_token=' . $this->session->data['user_token'] . '&customer_id=' . $this->request->get['customer_id']. $url, true)
        );

        if ($this->request->server['REQUEST_METHOD'] == 'POST') {
            $file = $_FILES['csv']['tmp_name'];
            $handle = fopen($file,"r");
            $this->model_customer_customer->deleteAllSpecialPrices();
            $imports = array();
            $current_row = 1;
            while ($data = fgetcsv($handle,1024)) 
            {
                ...
                else
                {
                    for ($c=0; $c < $number_of_fields; $c++)
                    {
                        $data_arr[$header_array[$c]] = $data[$c];
                    }					
                    $imports[$current_row] = $data_arr;	

                    $this->model_customer_customer->importCSV(($this->request->get['customer_id']), $data); 
        ...
        $this->response->setOutput($this->load->view('customer/customer_import_csv', $data));
    ...
    public function deleteImport() {
        ...
        $this->model_customer_customer->deleteImport($this->request->get['customer_import_id']);
        ...
        $this->response->redirect($this->url->link('customer/customer/edit', 'user_token=' . $this->session->data['user_token'] . $url . '/#tab-import', true));
